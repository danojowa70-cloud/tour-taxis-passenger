...
DROP TABLE IF EXISTS scheduled_rides CASCADE;

-- Create scheduled_rides table with all required columns
...
CREATE INDEX idx_scheduled_rides_passenger_id ON scheduled_rides(passenger_id);
...
DROP TRIGGER IF EXISTS scheduled_rides_updated_at ON scheduled_rides;
DROP FUNCTION IF EXISTS update_scheduled_rides_updated_at();

-- Create a trigger to automatically update the updated_at timestamp
CREATE FUNCTION update_scheduled_rides_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;

...
-- Create active_drivers table first (this should be a table, not a view)
...
CREATE TABLE IF NOT EXISTS driver_locations (
  driver_id UUID REFERENCES active_drivers(id) ON DELETE CASCADE,
...
-- Function to update driver location and status (missing function that code calls)
...
BEGIN
  ...
  -- Update last_seen in active_drivers to keep driver active
...
RETURNS VOID AS $$
BEGIN
  -- Update the active_drivers table
  UPDATE active_drivers 
  SET 
